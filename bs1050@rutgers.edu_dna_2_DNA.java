...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        StdIn.setFile(filename); // DO NOT remove this line, keep it as the first line in the metho
 


        int numberProfiles = 0;
        if (StdIn.hasNextLine()) {
            numberProfiles = Integer.parseInt(StdIn.readLine().trim());
        }
        database = new Profile[numberProfiles];
        for (int i = 0; i < numberProfiles; i++) {
            if (StdIn.hasNextLine()) {
             
           String name = StdIn.readLine().trim();
             
                String sequence1 = StdIn.hasNextLine() ? StdIn.readLine().trim() : "";
             
       String sequence2 = StdIn.hasNextLine() ? StdIn.readLine().trim() : "";

                
       database[i] = new Profile(name,null, null, sequence1, sequence2); // STR arrays set to null
     }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        int numberofSTRs = 0;
        if (StdIn.hasNextLine()) {
            numberofSTRs = Integer.parseInt(StdIn.readLine().trim()); 
        }
    
        STRsOfInterest = new String[numberofSTRs]; 
        for (int i = 0; i < numberofSTRs; i++) {
            if (StdIn.hasNextLine()) {
                STRsOfInterest[i] = StdIn.readLine().trim(); 
            }
    ...
    if (StdIn.hasNextLine()) {
        sequence1 = StdIn.readLine().trim();
    }
    
    if (StdIn.hasNextLine()) {
        sequence2 = StdIn.readLine().trim();
    }

    return new Profile("Unknown", null, null, sequence1, sequence2); // STR arrays set to null
}
    /**
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        int maximumofRepeats = 0;
        int currentofRepeats = 0;
        int index = 0;
    
        while (index < sequence.length()) {
            if (sequence.startsWith(STR, index)) {
                currentofRepeats++;
                index += STR.length();
            } else {
                maximumofRepeats = Math.max(maximumofRepeats, currentofRepeats);
                currentofRepeats = 0;
                index++;
            }
        }
        ...
        return new STR(STR, maximumofRepeats);
    }
    /**
